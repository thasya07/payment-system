...
Callback is idempotent, so multiple calls will not result in double charges
3. Notifications

After a successful callback, the system automatically calls:

Order Service: orderId + status
Notification Service: orderId, amount, accountNumber, status, email
Testing Flow
...
Payment Service generates a transactionId and sets status to PENDING
Trigger Payment Callback externally with payload:
{
  "status": "COMPLETED",
  "paymentMethod": "BANK_TRANSFER",
  "paidAt": "2026-03-27T22:30:00",
